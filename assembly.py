...
sha_translate = lambda num: (bin(num)[2:]).zfill(5) if num>=0 else bin(2**5-abs(num))[2:]
...
def MachineCodeTranslate(assemblecode):
    ...
    try:
        ...
        if a: return opcode[a.group(1)] + '00000' + register_translated(a.group(3)) + register_translated(a.group(2)) + sha_translate(int(a.group(4))) + funccode[a.group(1)]
        ...
        if a: return opcode['lui'] + '00000' + register_translated(a.group(1)) + imme_translated((lambda x: int(x, 16) if x.startswith('0x') else int(x, 10))(a.group(2)))
        a = re.match(r'(.*)[ ]+\$(.*)[ ]*,[ ]*([^\$]+)', assemblecode)                           #blez/bgtz/bltz $rs,label
        if a: return opcode[a.group(1)] + register_translated(a.group(2)) + '00000' + imme_translated((lambda x: int(x, 16) if x.startswith('0x') else int(x, 10))(a.group(3)))
...
instruction_set = []
with open('./data.txt') as f:                             #build instruction set
    for line in f:
        if(line.strip() != ''):
...
instruction_set = [instruction for instruction in instruction_set if instruction]          #A:
                                                                                           #    add ...
for (m,instruction) in enumerate(instruction_set):
    try:
